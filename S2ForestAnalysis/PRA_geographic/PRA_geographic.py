...
def PRA(forest_type, DEM, FOREST, SLOPE, ASPECT, radius, prob, winddir, windtol, pra_thd, sf):
    ...
    try:
        ...
        with rasterio.open('data/PRA_continuous.tif', "w", **dem_profile) as dest:
            dest.write(PRA)
        
        PRA = reclassify_PRA(PRA, dem_profile, pra_thd)
        
        # Save raster to path using meta data from dem.tif (i.e. projection)
        with rasterio.open('data/PRA_binary.tif', "w", **dem_profile) as dest:
            dest.write(PRA)
...
PRA('sen2cc', 'dem.tif', 'forest_ccp.tif', 'slope.tif', 'aspect.tif', 6, 0.5, 0, 180, 0.15, 3)
